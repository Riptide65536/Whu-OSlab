...
// Disk inode structure (64 bytes on disk)
struct dinode {
...
inode_t* inode_get(uint32 dev, uint16 inum);       // 在内存申请或查询inode(ref++)
inode_t* inode_alloc(uint32 dev, uint16 type); // 在磁盘里创建新的inode并在内存申请对应副本
