...
void log_init(int dev, struct superblock *sb) {
    ...
    spinlock_init(&log_ctx.lock, "log"); // 确保锁也被初始化了
    log_ctx.dev = dev;
    log_ctx.start = sb->logstart;
    log_ctx.size = sb->nlog;

    // printf("log_init: zeroing log\n");
    for (int i = 0; i < log_ctx.size; i++) {
        struct buf *bp = bread(log_ctx.dev, log_ctx.start + i);
        memset(bp->data, 0, BSIZE);
        bwrite(bp); // <-- 使用 bwrite 进行直接、非日志的写操作
        brelse(bp);
    }
    // printf("log_init: zeroing log done\n");
